...
# lookup(ReAct 조회 루프) 모드에서 통째로 제외할 테이블.
# 벡터 검색은 execute_vector_sql 고정 템플릿이 전담하므로, ReAct LLM에게
# emb_rslt(vector) 컬럼을 노출하면 직접 SELECT를 시도해 할루시네이션이 발생한다.
_LOOKUP_EXCLUDE_TABLES = ("int_com_file_emb",)


def _to_lookup_entry(entry: str) -> str:
    """단일 스키마 항목을 경량 버전으로 축약한다. 규칙 전용·제외 테이블 블록이면 빈 문자열."""
    if entry.lstrip().startswith("-- [INSERT 규칙]"):
        return ""
    # 제외 대상 테이블 헤더로 시작하는 항목은 통째로 드롭
    head = entry.lstrip()
    if any(head.startswith(f"-- {tbl}") for tbl in _LOOKUP_EXCLUDE_TABLES):
        return ""
    kept: list[str] = []
    skipping = False
