...
async def node_react_gather(state: GraphState) -> Command:
    ...
    from app.hj.services.tools import execute_select_tool, execute_sparql_tool, execute_vector_sql, VECTOR_SEARCH_SQL, VECTOR_SEARCH_INTENTS
    from app.hj.services.sparql_schema import get_sparql_schema, GRAPH_RAG_INTENTS
    ...
    # ── 조회 백엔드 선택 (상호배타) ────────────────────────────────────────────
    # search(순수 조회) + Fuseki 적재 도메인(veh/mtgr/leave) → SPARQL 전용(추론).
    # 그 외(excu 쓰기 전 수집, 미적재 도메인) → 기존 SQL 경로 유지.
    use_graph = (
        settings.fuseki_rag_enabled
        and action_type == "search"
        and intent in GRAPH_RAG_INTENTS
    )
    if use_graph:
        tools_list = [execute_sparql_tool]
        tools_map = {"execute_sparql_tool": execute_sparql_tool}
    else:
    ...
    if use_graph:
        _tool_guide = (
            "- 오직 execute_sparql_tool(Fuseki SPARQL, 읽기 전용)만 사용하세요. SQL 도구는 제공되지 않습니다.\n"
            "  관계 탐색(예약-차량/회의실, 휴가 결재선)과 상위개념 추론(리즈너가 하위 인스턴스 자동 포함)에 활용하세요.\n"
            "  (INSERT/DELETE 등 SPARQL Update 는 제공되지 않습니다)\n"
        )
    else:
