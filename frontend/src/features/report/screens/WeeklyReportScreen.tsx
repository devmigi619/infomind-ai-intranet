...
import { useReportList, useCreateReport, type Report } from '../api';
...
export function WeeklyReportScreen() {
  ...
  const { data: reports = [], isLoading, error } = useReportList(0);
  ...
  const handleSave = async () => {
    ...
    try {
      await createReport.mutateAsync({ weekStart, thisWeek, nextWeek, issues: issues || null });
      setWeekStart('');
      setThisWeek('');
      setNextWeek('');
      setIssues('');
