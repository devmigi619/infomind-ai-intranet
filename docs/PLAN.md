...
### ✅ 협업 환경 정비
- [x] 모듈별 README (인터페이스 명세)
- [x] features/README.md (협업 가이드)
...
- [ ] 사용자 설정 화면 (프로필/비밀번호/알림 카테고리별)
  - [x] SettingsScreen 골격 (풀뷰, 사이드바 4 카테고리)
  - [x] 계정 섹션 UI (이메일/연락처 편집, 비밀번호 변경 placeholder)
  - [x] 알림 섹션 UI (마스터/카테고리별 토글)
  - [x] 맞춤설정 섹션 UI (NavRail 편집 진입, 위젯/결재선 placeholder)
  - [x] 화면 섹션 UI (다크모드/언어 placeholder)
  - [ ] Track 2: 이메일/연락처/비밀번호 API 연결 (백엔드 정리 후)
- [ ] 로그인 화면 다듬기

### 알림 시스템
- [ ] Track 1: 인앱 벨 + 드롭다운 (프론트 골격, 목 데이터)
- [ ] Track 2: 백엔드 알림 API + 트리거 (동료 PR 후)
- [ ] Track 3: FCM 푸시 연동 (Phase 2)
- [ ] Track 4: 풀뷰 알림 센터 (Phase 2)
