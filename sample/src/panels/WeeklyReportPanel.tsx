  ...
  return (
    <div className="p-6">
      ...
      <select
        ...
        className="w-full h-9 px-3 text-small bg-white rounded-md outline-none mb-4 cursor-pointer"
      ...
      <div className="flex items-center gap-2 mb-4 px-1">
        ...
        <span className="text-small text-black">
          제출 현황: <strong>{submittedCount}/{totalCount}</strong>
        </span>
      </div>
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <div className="flex items-center gap-3 mt-2">
                  ...
                  {report.submittedAt && (
                    <span className="text-micro text-mori-muted">제출: {report.submittedAt}</span>
                  )}
                </div>
              </div>
              <div className="flex-shrink-0 ml-2">
                {report.status === 'submitted' ? (
                  <CheckCircle2 size={18} className="text-[#43A047]" />
                ) : (
                  <Circle size={18} className="text-[#FB8C00]" />
                )}
              </div>
            </div>
          </div>
