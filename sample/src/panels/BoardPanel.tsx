...
function PostDetail({ post, onBack }: { post: BoardPost; onBack: () => void }) {
  return (
    <div className="p-6">
      ...
      <button
        onClick={onBack}
        className="flex items-center gap-1.5 text-small text-mori-body hover:text-[#0A2463] transition-colors mb-4"
      >
        <ArrowLeft size={16} />
        목록으로
      </button>
      ...
      {/* Category Tag */}
      <span
        className="inline-block text-micro px-2 py-0.5 rounded mb-3"
      ...
      <div className="flex items-center gap-3 mb-6 pb-4" style={{ borderBottom: '1px solid rgba(0, 0, 0, 0.08)' }}>
        <div
          className="w-8 h-8 rounded-full flex items-center justify-center text-white text-micro font-medium"
        ...
        <div className="ml-auto flex items-center gap-3">
          <span className="flex items-center gap-1 text-micro text-mori-muted">
            <Eye size={12} />
      ...
      <div className="flex flex-wrap gap-2 mb-4">
        {categories.map((cat) => (
          <button
            ...
            className="px-3 py-1.5 rounded-full text-micro transition-all duration-200"
            style={{
              background: activeCategory === cat ? '#0A2463' : 'transparent',
              color: activeCategory === cat ? '#FFFFFF' : 'rgba(0, 0, 0, 0.55)',
      ...
      <div className="space-y-2">
        {filteredPosts.map((post) => (
          <button
            ...
            className="w-full text-left bg-[#F9F9F9] rounded-md p-4 hover:shadow-card transition-all duration-200"
          >
            <span
              className="inline-block text-micro px-2 py-0.5 rounded"
              style={{ background: 'rgba(10, 36, 99, 0.08)', color: '#0A2463' }}
            >
              {post.category}
            </span>
            ...
            <div className="flex items-center justify-between mt-2">
              ...
              <span className="text-micro text-mori-muted">
                조회 {post.views.toLocaleString()} · 댓글 {post.comments}
              </span>
            </div>
          </button>
        ))}
